...
## LLM Models: GPT 4o, **Claude 3 Sonnet and Mistral 7B**
...
# promptv1_gpt <- add_suffix(promptv1_gpt, "promptv1_gpt")
# promptv1_claude <- add_suffix(promptv1_claude, "promptv1_claude")
# promptv1_mistral <- add_suffix(promptv1_mistral, "promptv1_mistral")

# Make sure the ids are numeric instead of characters
promptv1_gpt <- promptv1_gpt |> 
  mutate(respondent_id = as.numeric(respondent_id))

promptv1_claude <- promptv1_claude |> 
  mutate(respondent_id = as.numeric(respondent_id))

promptv1_mistral <- promptv1_mistral |> 
  mutate(respondent_id = as.numeric(respondent_id))
```

```{r}
tidy_join_transcript5<- tidy_join_transcript4 |> 
  inner_join(promptv1_gpt,by="respondent_id") |> 
  inner_join(promptv1_claude,by="respondent_id") |> 
  inner_join(promptv1_mistral,by="respondent_id") 
```

#### Performance: prompt v1 VS baseline model
